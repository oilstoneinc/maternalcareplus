...
export async function ensureMCHSchema() {
  ...
  const statements = [
    ...
    sql`ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "insurance_provider" text`,
    sql`ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "insurance_policy_number" text`,
    sql`ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "insurance_expiry_date" timestamp`,
